...
CREATE TABLE IF NOT EXISTS brands (
    ...
    mailketing_api_token VARCHAR(255) NULL,
    theme_preset ENUM(
        'gold',
        'silver',
        'bronze',
...
CREATE TABLE IF NOT EXISTS referrers (
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uniq_brand_event_ref (
        brand_id,
        event_slug,
        ref_code
    ),
...
ALTER TABLE events
ADD COLUMN IF NOT EXISTS reward_image VARCHAR(255) NULL AFTER event_capacity;

ALTER TABLE events
ADD COLUMN IF NOT EXISTS meta_pixel_id VARCHAR(50) NULL AFTER reward_image;

ALTER TABLE events
ADD COLUMN IF NOT EXISTS ga_measurement_id VARCHAR(30) NULL AFTER meta_pixel_id;

ALTER TABLE events
ADD COLUMN IF NOT EXISTS flyer_path VARCHAR(255) NULL AFTER reward_image;

ALTER TABLE events
ADD COLUMN IF NOT EXISTS event_date DATE NULL AFTER event_day;

ALTER TABLE referrers
ADD COLUMN IF NOT EXISTS brand_id INT NULL AFTER id;

ALTER TABLE referrers
ADD COLUMN IF NOT EXISTS event_slug VARCHAR(60) NOT NULL DEFAULT 'default' AFTER ref_code;
...
VALUES (
        1,
        'default',
        'Rahasia Emas — Acara Utama',
        'active',
        '628111111111',
        'Jumat, 25 Juli 2026',
        '19.30 WIB',
        'Online via Zoom (link dikirim via WhatsApp)',
        'Coach Arifin',
        '100'
    )
ON DUPLICATE KEY UPDATE
